...
from transformers import AutoModelForCausalLM, AutoTokenizer, QuantoConfig, HfArgumentParser
...
def main(args, dpo_config):
    ...
    def process(row): # depends on the dataset!
        if "hassanjbara" in args.dataset_name:
            row["prompt"] = tokenizer.apply_chat_template([{"content": row["query"], "role": "user"}], tokenize=False)
